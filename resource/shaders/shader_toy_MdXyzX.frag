#version 450 core
// copy from: https://www.shadertoy.com/view/MdXyzX
// amazing!
...
vec3 getRay(vec2 uv){
    ...
    if(Resolution.x < 400.0) return proj;
        vec3 ray = rotmat(vec3(0.0, -1.0, 0.0), 3.0 * ((Mouse.x + 0.5) * 2.0 - 1.0)) * rotmat(vec3(1.0, 0.0, 0.0), 1.5 * ((Mouse.y + 0.5) * 2.0 - 1.0)) * proj;
    return ray;
}
