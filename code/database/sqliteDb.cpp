...
#include <iostream>
sqliteDb::sqliteDb() { dbDir = QDir::currentPath(); }
sqliteDb::~sqliteDb() {}
// 设置数据库存放路径
...
bool sqliteDb::getData(QString dbName, QString tableName, QList<QHash<QString, QString>> &data, QString sqlWhere) {
    ...
    if (!sqlWhere.isEmpty()) queryStr += " " + sqlWhere;
...
// 获取数据
bool sqliteDb::getData(QString dbName, QString tableName, QHash<QString, QString> columndata,
                       QList<QHash<QString, QString>> &data, QString sqlWhere) {
    QString colunmStr;
    if (columndata.count() == 0)
        ...
        for (auto key : keys) {
            ...
            if (!colunmStr.isEmpty()) colunmStr += ",";
            colunmStr += column;
        }
    }
...
// 获取数据
bool sqliteDb::getData(QString tableName, QHash<QString, QString> columndata, QList<QHash<QString, QString>> &data,
                       QString sqlWhere) {
    if (databaseName.isEmpty()) return false;
