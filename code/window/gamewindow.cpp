...
const QPoint opengl_down_right =
    opengl_up_left + QPoint(hypercube_size.x(), hypercube_size.y());
...
GameWindow::GameWindow(QWidget *parent)
  ...
  */
  ...
  connect(timer_flush_score_and_left_time_bar_, &QTimer::timeout, [&]() {
    // score_bar
    ui->score_bar->setText(
        QString::fromStdString(std::to_string(board->GetScore())));
    // left_time_bar
  ...
  int font_Id =
      QFontDatabase::addApplicationFont(":/font/SmileySans-Oblique.ttf");
...
void GameWindow::keyPressEvent(QKeyEvent *e) {
  ...
  if (e->key() == Qt::Key_B) {  // B键，是否启用BlinnPhong光照模型
    ...
    std::cout << "Blinn Phong: " << (shader_blinn_phong ? "true" : "false")
              << std::endl;
  ...
  if (e->key() == Qt::Key_E || e->key() == Qt::Key_R) {
    if (e->key() == Qt::Key_E)
      shader_render_mode =
          (shader_render_mode + 4) % 5;  // E键，切换渲染模式，前翻
    else
      shader_render_mode =
          (shader_render_mode + 1) % 5;  // R键，切换渲染模式，后翻
...
void GameWindow::on_hint_button_pressed() {
  ui->hint_button->setIcon(QIcon(":/images/gamewindow/5-.png"));
}
...
void GameWindow::on_btnReturn_pressed() {
  ui->btnReturn->setIcon(QIcon(":/images/gamewindow/6-.png"));
}
