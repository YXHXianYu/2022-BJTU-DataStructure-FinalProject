...
void Board::InitHypercube() {
  ...
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      // std::cout << i << " " << j << " " << stones_[i][j].GetType() <<
      // std::endl;
      std::cout << i << " " << j << " "
                << hypercube_->stone_manager_->Generate(
                       i, j, stones_[i][j].GetType(), rand() % 500)
                << std::endl;
    }
  }
}
  ...
  if (start) {
    while (Check()) {
      ...
      for (auto match : matches_) {
        ...
        stones_[match.first][match.second].SetType(
            rand() % Stone::GetMaxType() + 1);
      }
      // Fall2();
      add_tools = 0;
    }
...
void Board::Clicked(int x, int y) {
  ...
  for (int i = 0; i < 8; ++i) {
    for (int j = 0; j < 8; ++j) {
      if (x >= positions_[i][j].first &&
          x <= positions_[i][j].first + length_ &&
          y >= positions_[i][j].second &&
  ...
  */
  ...
  if (chosen_.first == -1) {
    if (mouse_on_diamond) {
      stones_[chosen_x][chosen_y].SetType(
          stones_[chosen_x][chosen_y].GetMaxType() + 1);
      mouse_on_diamond = 0;
      rest_diamond--;
      return;
    }
